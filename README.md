...
- **Stripe sandbox**: commerce stays local unless `COMMERCE_SOURCE=stripe` and `STRIPE_SANDBOX_ENABLED=true` are set with a test restricted key, `local-demo` tenant, account id, and webhook secret. The adapter pins Stripe API `2026-08-26.dahlia`, uses only `https://api.stripe.com` outside tests, rejects live keys/resources, and persists the selected commerce and transaction bindings when a case is accepted. Its restricted key needs read access to the Account endpoint as well as Customers, Checkout Sessions, PaymentIntents, Subscriptions, Invoices, Invoice Payments and Refunds, and write access only to Refunds and Subscriptions. Subscription writes are limited to a verified owner's explicit no-refund request for `cancel_at_period_end=true`; immediate cancellation, proration, credits, and invoice changes are not performed. Unknown or failed scheduling is escalated and never reported as cancelled.
- **Stripe refunds and recovery**: an approved native `issue_refund` command first writes a leased attempt and uses its stable Stripe idempotency key. A created refund is **pending** until Stripe reports or the adapter retrieves a terminal state; pending never generates an issued/refunded customer claim. `refund.created`, `refund.updated`, and `refund.failed` webhooks require bounded raw JSON, Stripe HMAC/timestamp, test mode, configured account, and pinned API version before parse. Reconciliation retrieves the refund again, so a late legitimate failure can correct an earlier success observation and escalate once; unrelated signed events have no effect. An unknown POST outcome is never retried under a fresh key after Stripe's 24-hour idempotency window.
...
Stripe sandbox calls and mutations also require explicit human authorization. Use only synthetic test Customer/Checkout/Subscription data, a restricted `rk_test_` key, and the approved validation budget. Keep webhook receipts and identifiers redacted; do not run tests with inherited Stripe credentials.
