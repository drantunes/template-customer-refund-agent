...
import { supportCasesListRoute, supportInboundRoute } from '../../src/mastra/server/routes';
...
describe('support API WIP characterization', () => {
  it('rejects malformed inbound JSON at the HTTP boundary', async () => {
    ...
    expect(response).toEqual({ body: { error: 'Invalid JSON body.' }, status: 400 });
  });
  ...
  it('returns the list envelope used by the demo UI', async () => {
    ...
    expect(response).toMatchObject({ status: 200, body: { cases: expect.any(Array) } });
  });
});
