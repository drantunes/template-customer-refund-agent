...
async function loadCharacterizationRuntime(draft: {
    ...
    return {
      ...
      ModelRouterEmbeddingModel: class DeterministicEmbeddingModel {
        async doEmbed({ values }: { values: string[] }) {
          return { embeddings: values.map(() => Array.from({ length: 1536 }, () => 0)) };
        }
      },
    };
  });

  const [{ mastra }, { caseStore }, { mockSupportAdapter }, { triageAgent }, { responseAgent }, { searchSupportKnowledgeTool }] =
    await Promise.all([
  ...
  const supportCase = await caseStore.create({ id: `case_${crypto.randomUUID()}`, status: 'new', ...normalized });

  return { mastra, caseStore, supportCase };
}
...
describe('resolve support case WIP characterization', () => {
  it('suspends a refund recommendation, then resolves after the existing workflow approval checkpoint', async () => {
    ...
    const suspended = await run.start({ inputData: { caseId: supportCase.id } });
    expect(suspended.status).toBe('suspended');
    expect((await caseStore.get(supportCase.id))?.status).toBe('waiting_approval');
  ...
  it('escalates when the existing approval checkpoint is rejected', async () => {
    ...
    const resumed = await run.resume({
      step: 'request-approval',
      resumeData: { approved: false, approverId: 'characterization-approver', note: 'Needs review.' },
    });
  ...
  it('reports a failed workflow when the deterministic triage double fails', async () => {
    ...
    vi.mocked(triageAgent.generate).mockRejectedValueOnce(new Error('Synthetic triage failure.'));
    const run = await mastra.getWorkflow('resolveSupportCaseWorkflow').createRun();
  ...
  it('persists an inbound case once and returns the same case for a duplicate event', async () => {
    ...
    const first = await (await workflow.createRun()).start({ inputData: { payload } });
    const second = await (await workflow.createRun()).start({ inputData: { payload } });

    expect(first.status).toBe('success');
    expect(second.status).toBe('success');
    expect(first.result.caseId).toBe(second.result.caseId);
    expect((await caseStore.list()).filter(supportCase => supportCase.externalId === payload.externalId)).toHaveLength(1);
