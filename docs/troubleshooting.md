...
If `npm run check:env -- --profile=local` fails, set a unique `LOCAL_AUTH_SIGNING_KEY` of at least 32 characters in `.env`. Keep `SUPPORT_SOURCE=mock` and `COMMERCE_SOURCE=mock` for the local demo. Provider profiles validate every enabled provider, so selecting a named profile cannot hide incomplete Intercom or Stripe configuration.
...
The local portal is served on `http://localhost:5173` and proxies `/support/*` to the backend on `http://localhost:4111`. Start `npm run dev` first, then `npm run --workspace support-refund-agent-web dev`. The browser E2E suite chooses an isolated temporary database and mock providers; it requires the Playwright Chromium runtime already installed.
