...
The local portal is served on `http://localhost:5173/portal` and proxies `/support/*` to the backend on `http://localhost:4111`. Start `npm run dev` first, then `npm run --workspace support-refund-agent-web dev`. The browser E2E suite chooses an isolated temporary database and mock providers; it requires the Playwright Chromium runtime already installed.
