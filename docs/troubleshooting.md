...
In the local `npm run dev` Studio, no sign-in is required: it is bound to loopback and receives the scoped `local-demo` support-agent view. Registered workflow graphs and **Recent runs** remain tenant-filtered. Any explicit invalid, customer, approver, or foreign-tenant Bearer token is denied, and direct tool/workflow execution remains unavailable. SQLite keeps history across restarts; inbound snapshots expire after seven days and case-linked workflow history after 90 days. Support routes and refund writes still require their normal authenticated application approval.
