...
export class CaseStore {
  ...
  async migrate(target = 4): Promise<void> {
  ...
  async create(case_: SupportCase) {
    ...
    for (const message of persisted.messages)
      await this.appendMessageRow(persisted.id, message);
    return persisted;
  }
  async update(
    id: string,
    ...
    if (patch.messages)
      for (const message of patch.messages)
        await this.appendMessageRow(id, message);
    return updated;
  }
  async appendMessage(id: string, message: CaseMessage) {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      const read = await this.client.execute({
        sql: "SELECT data, version FROM support_cases WHERE id = ?",
        args: [id],
      });
  ...
  async claimDispatch(limit = 10): Promise<DispatchRecord[]> {
    ...
    for (const row of rows.rows) {
      const update = await this.client.execute({
        sql: "UPDATE support_dispatch SET state = 'claimed', attempts = attempts + 1, lease_until = ?, updated_at = ? WHERE id = ? AND (state = 'pending' OR (state IN ('claimed', 'started') AND lease_until < ?))",
        args: [leaseUntil, claimedAt, String(row.id), claimedAt],
      });
  ...
  async completeDispatch(
    ...
    await this.client.execute({
      sql: "UPDATE support_dispatch SET state = ?, lease_until = NULL, last_error = ?, updated_at = ? WHERE id = ?",
      args: [state, error ? String(error) : null, now(), id],
    });
  }
  async markDispatchStarted(caseId: string) {
    await this.ensured();
    await this.client.execute({
      sql: "UPDATE support_dispatch SET state = 'started', updated_at = ? WHERE case_id = ? AND state = 'claimed'",
      args: [now(), caseId],
    });
  }
  ...
  async claimDispatchForStart(
    ...
    const update = await this.client.execute({
      sql: "UPDATE support_dispatch SET state = 'claimed', attempts = attempts + 1, lease_until = ?, updated_at = ? WHERE case_id = ? AND state = 'pending'",
      args: [leaseUntil, claimedAt, caseId],
    });
  ...
  async claimOutbox(limit = 10) {
    ...
    for (const row of rows.rows) {
      const changed = await this.client.execute({
        sql: "UPDATE support_outbox SET state = 'claimed', attempts = attempts + 1, lease_until = ?, updated_at = ? WHERE id = ? AND (state = 'pending' OR (state = 'claimed' AND lease_until < ?))",
        args: [leaseUntil, claimedAt, String(row.id), claimedAt],
      });
  ...
  async completeOutbox(id: string, receipt: unknown) {
    await this.client.execute({
      sql: "UPDATE support_outbox SET state = 'delivered', receipt = ?, lease_until = NULL, updated_at = ? WHERE id = ?",
      args: [JSON.stringify(receipt), now(), id],
    });
  }
  async retryOutbox(id: string, error: unknown, terminal = false) {
    await this.client.execute({
      sql: "UPDATE support_outbox SET state = ?, last_error = ?, updated_at = ? WHERE id = ?",
      args: [terminal ? "failed" : "pending", String(error), now(), id],
    });
  }
  private outbox(
