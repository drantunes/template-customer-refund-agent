...
export function databaseProfile(environment = process.env) {
  ...
  return {
    ...
    client: local
      ? absoluteFileUrl(
          value(environment, "LOCAL_DEMO_CLIENT_DATABASE_URL"),
          localClientDefault,
        )
...
export function assertDatabaseIsolation(environment = process.env) {
  ...
  const localBackend = absoluteFileUrl(
    value(environment, "LOCAL_DEMO_DATABASE_URL"),
    localBackendDefault,
  );
  const localClient = absoluteFileUrl(
    value(environment, "LOCAL_DEMO_CLIENT_DATABASE_URL"),
    localClientDefault,
  );
  ...
  const localPaths = [filePath(localBackend), filePath(localClient)].filter(
    Boolean,
  );
  const externalPaths = [
    filePath(externalBackend),
    filePath(externalClient),
  ].filter(Boolean);
  if (
    localPaths.some((path) =>
      externalPaths.some((other) => sameFile(path, other)),
    )
  )
    throw new Error(
      "Local and external database profiles must use different files.",
    );
  if (isLocalMode(environment)) {
    if (
      (!selected.backend.startsWith("file:") ||
        selected.backend.includes(":memory:")) &&
      environment.VITEST !== "true"
    )
      throw new Error(
        "LOCAL_DEMO_DATABASE_URL must be a persistent file: URL.",
      );
    if (
      (!selected.client.startsWith("file:") ||
        selected.client.includes(":memory:")) &&
      environment.VITEST !== "true"
    )
      throw new Error(
        "LOCAL_DEMO_CLIENT_DATABASE_URL must be a persistent file: URL.",
      );
    if (sameFile(filePath(selected.backend), filePath(selected.client)))
      throw new Error(
        "Local backend and client databases must use different files.",
      );
  }
  if (
